    ...
    for (int h = 0; h < height; h++) {
        for (int w = 0; w < width; w++) {
            for (int k = 0; k < anchor_num; k++) {
                ...
                RCLCPP_INFO(rclcpp::get_logger("obstacledetection"), "index box_center_x: %f center_y:%f scale_x:%f scale_y:%f", center_x, center_y, scale_x, scale_y);
...
{   
    ...
    //根据偏移获得实时的调节率，偏移越小调节率越大（对应转弯角速度越大）
    return .0;
}
...
int Football_node::PostProcess(
    ...
    // play_football源码
//     if(exist_result == 1){
//             message.angular.z = -1.0 * (target_point - 480) / 300.0;            //设置角速度
//             message.linear.x    = 0.24;                                                                         //以固定的线速度向前运动
//     }
//     else {
//             message.angular.z = 0.15;                                                                         //原地转圈
//             message.linear.x    = 0;
//     }
